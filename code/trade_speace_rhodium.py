...
def farm_approach2(node, numOfUsers, latitude, crop_type, sampEn, rainfall):
...
# Setup the model
def setupModel(func, nodes, input_numOfUsers, input_latitude, input_sampleEn, input_rainFall, input_cropType):
...
# Policy Evaluation
def policyEval(model, nodes, numOfUsers, latitude, sampEn, rainfall, crop_type):
...
def SA(model, nodes, numOfUsers, latitude, sampEn, rainfall, crop_type):
